...
UPDATE `creature_template` SET `faction`=16, `BaseAttackTime`=250, `unit_flags`=33554944, `unit_flags2`=2097152, `unit_flags3`=27787264 WHERE `entry`=202167; -- Ray of Anguish
...
DELETE FROM `areatrigger_create_properties` WHERE `Id` IN (28185, 28466, 28022, 28429);
...
(28022, 31941, 0, 0, 0, 0, -1, 0, 0, 0, 0, 4, 0.75, 0.75, 10, 10, 0.300000011920928955, 0.300000011920928955, 0, 0, '', 49679), -- Spell: 402219 (Ray of Anguish)
...
DELETE FROM `areatrigger_template` WHERE (`IsServerSide`=0 AND `Id` IN (32079, 32636, 32306, 31941, 32313));
...
DELETE FROM `spell_script_names` WHERE `spell_id`IN (402538, 402547, 404744, 404743, 406525, 406517, 407196, 402260, 407068);
...
(407068, 'spell_kazzara_rays_of_anguish_rift_selector');
