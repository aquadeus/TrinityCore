...
void Spell::EffectSetChromieTime()
{
    ...
    if (!effectInfo->MiscValue)
        caster->SetChromieTimeExpansion(0, 0, 0, 0); // Selected the Present
    else
    {
        UiChromieTimeExpansionInfoEntry const* expansion = sUiChromieTimeExpansionInfoStore.LookupEntry(effectInfo->MiscValue);
        uint32 contentTuningId = sDB2Manager.GetRedirectedContentTuningId(expansion->ContentTuningID, contentTuningId->RedirectFlag);
        caster->SetChromieTimeExpansion(expansion->ID, expansion->ExpansionLevelMask, caster->m_playerData->CtrOptions->Field_4, caster->m_playerData->CtrOptions->ContentTuningConditionMask);
    }
}
